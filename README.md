...
| ppt-storyboard | `/jxin-writing:ppt` | `/jxin-writing:ppt-storyboard` | PPT 叙事分镜。三幕式叙事弧线，根据受众（管理层/技术/投资方）适配内容密度，可选择 GPT 或 Gemini 生成分镜图片并合成 PDF |
| gemini-image-gen | `/jxin-writing:image`（可选 Gemini） | `/jxin-writing:gemini-image-gen` | Gemini 图片生成。通过 Gemini Web API 生成图片，支持中文提示词，内置频率控制。也供其他技能内部调用 |
| gpt-image-gen | `/jxin-writing:image`（可选 GPT） / `/jxin-writing:gpt-image` | `/jxin-writing:gpt-image-gen` | GPT 图片生成。通过 Responses API 调用 gpt-image-2，使用项目级 host/key 配置，也供其他技能内部调用 |
...
/jxin-writing:image           # 生成图片，可选择 GPT 或 Gemini
/jxin-writing:gpt-image       # 使用 GPT 生成图片
