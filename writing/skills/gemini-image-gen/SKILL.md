...
- `--output` / `-o`：输出图片路径（可选，默认根据提示词文件位置自动推断，并兼容 `smart-image-generator-output` 旧目录约定）
...
- Profile：`~/Library/Application Support/gemini-image-gen/chrome-profile/`（macOS）
- 可通过环境变量覆盖：`GEMINI_WEB_COOKIE_PATH`、`GEMINI_WEB_CHROME_PROFILE_DIR`、`BAOYU_CHROME_PROFILE_DIR`
